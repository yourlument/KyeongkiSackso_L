...
export function CommunityDetailView({ post, viewer }: { post: CommunityDetailPost; viewer: DetailViewer }) {
  const active = post.status === "진행중";
  const [comments, setComments] = useState<CommunityDetailComment[]>(post.comments);
  const [answerCount, setAnswerCount] = useState(post.answers);

  const addAnswer = (c: CommunityDetailComment) => {
    ...
    <div style={{ maxWidth: "839px", margin: "0 auto" }}>
      ...
      <article style={{ borderRadius: "19.52px", border: "1px solid rgba(210,210,215,0.2)", background: "#fff", padding: "30.28px" }}>
        ...
        <div className="flex items-center flex-wrap" style={{ gap: "14.64px", marginTop: "19.52px", paddingBottom: "20.52px", borderBottom: "1px solid rgba(210,210,215,0.15)" }}>
          ...
          <Meta icon={<CommentIcon />}>업체 답변 {answerCount}건</Meta>
        </div>
      ...
      <section style={{ marginTop: "24.4px", borderRadius: "19.52px", border: "1px solid rgba(210,210,215,0.2)", background: "#fff", overflow: "hidden" }}>
        <div className="flex items-center justify-between" style={{ padding: "19.52px 29.28px 20.52px" }}>
          <p className="flex items-center" style={{ gap: "5px", margin: 0 }}>
            ...
            <span style={{ fontSize: "15px", fontWeight: 700, letterSpacing: "-0.225px", lineHeight: "27px", color: NAVY }}>{answerCount}</span>
          </p>
        ...
        {viewer === "official" && comments.length === 0 && (
          <div style={{ background: "#FAFAFA", borderTop: "1px solid rgba(210,210,215,0.15)", padding: "20.52px 29.28px 19.52px" }}>
        ...
        {viewer === "supplier" && active && (
          <AnswerForm postId={post.id} onSubmit={addAnswer} />
        )}
      </section>
    </div>
  );
...
function CommentRow({ c, index, viewer, postId, onReply }: {
  ...
  const submitReply = async () => {
    ...
    if (res.ok) {
      ...
      onReply(c.id, { name: data.name, date: data.date, body: replyText });
      setReplyText("");
      setShowForm(false);
    }
    setSubmitting(false);
  ...
  return (
    <div style={{ padding: index === 0 ? "24.4px 29.28px" : "25.4px 29.28px 24.4px", borderTop: index === 0 ? "none" : "1px solid rgba(210,210,215,0.1)" }}>
      ...
      {showForm && (
        <div style={{ marginTop: "14.64px", marginLeft: "58.56px" }}>
          <textarea
            value={replyText}
            onChange={(e) => setReplyText(e.target.value)}
            placeholder="답글을 입력하세요..."
            style={{ width: "100%", borderRadius: "14.64px", border: "1px solid rgba(210,210,215,0.3)", padding: "13.19px 18.08px", fontSize: "13px", color: INK, resize: "vertical", minHeight: "80px", outline: "none", background: "#fff" }}
          />
          <div style={{ marginTop: "7.32px", textAlign: "right" }}>
            <button type="button" onClick={submitReply} disabled={submitting || !replyText.trim()}
              style={{ borderRadius: "12px", background: NAVY, color: "#fff", border: "none", padding: "8px 20px", fontSize: "12px", fontWeight: 600, cursor: "pointer", opacity: (!replyText.trim() || submitting) ? 0.5 : 1 }}>
  ...
  return (
    <div style={{ borderTop: "1px solid rgba(210,210,215,0.15)", padding: "20.52px 29.28px" }}>
      <textarea
        ...
        placeholder="업체 답변을 입력하세요..."
        style={{ width: "100%", borderRadius: "14.64px", border: "1px solid rgba(210,210,215,0.3)", padding: "14.64px 18.08px", fontSize: "13px", color: INK, resize: "vertical", minHeight: "100px", outline: "none", background: "#fff" }}
      />
