...
export async function POST(req: Request) {
  ...
  if (action === "approve") {
    await prisma.supplierCompany.update({ where: { id }, data: { approvalStatus: "APPROVED", approvalMemo: null } });
    return NextResponse.json({ status: "승인" });
  }
  await prisma.supplierCompany.update({ where: { id }, data: { approvalStatus: "REJECTED", approvalMemo: reason ?? null } });
