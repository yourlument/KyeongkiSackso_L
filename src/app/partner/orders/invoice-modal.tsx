...
export function InvoiceModal({
  ...
  order: OrderRow;
  onClose: () => void;
  onSubmit: (courier: string, trackingNo: string) => void;
}) {
  const [courier, setCourier] = useState("");
  const [invoice, setInvoice] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const canSubmit = courier !== "" && invoice.trim().length > 0 && !submitting;
      ...
      >
        ...
        <div className="flex" style={{ gap: "14.64px", paddingTop: "29.28px" }}>
          ...
          <button
            type="button"
            disabled={!canSubmit}
            onClick={() => { setSubmitting(true); onSubmit(courier, invoice.trim()); }}
